...
using static Food_Scape.Services.ReCAPTCHA;
           ...
           // Recaptcha Ends here
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    _context.FsUsers.Add(newUser);
                    _context.SaveChanges();

                    AccountInfoRepository accountInfoRepo = new AccountInfoRepository(_context);
                    FsUser userNew = accountInfoRepo.GetUserByEmail(Input.Email);
                    HttpContext.Session.SetInt32("CLIENT_ID_KEY", userNew.UserId);
